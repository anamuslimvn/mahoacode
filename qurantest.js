...
async function fetchRandomSura() {
  ...
  const response = await fetch(`${baseUrl}${currentLanguage}/${suraNumber}.json`);
...
async function displayRandomVerse() {
  ...
  currentVerseDetails = {
    sura: randomVerse.SURAH,
    aya: randomVerse.AYAH,
  ...
  verseDiv.innerHTML = `
    <div class="ayat">
      <div class="language-buttons">
        ...
        <button class="language-button" id="btnEn" onclick="loadTranslation('english_saheeh')">En</button>
      </div>
      <div class="ayat-details">Quran ${randomVerse.SURAH}:${randomVerse.AYAH}</div>
    ...
    <p class="quranarrandom">${randomVerse.ARABIC_TEXT} <span style="color: #c91003;">${toArabicNumeral(randomVerse.AYAH)}</span></p>
  ...
  document.getElementById('audioPlayer').src =
    `${audioBaseUrl}${randomVerse.SURAH.toString().padStart(3, '0')}${randomVerse.AYAH.toString().padStart(3, '0')}.mp3`;
}
...
async function loadTranslation(language) {
  ...
  const response = await fetch(`${baseUrl}${currentLanguage}/${suraNumber}.json`);
  const data = await response.json();
  const verse = data.find(v => v.AYAH == ayaNumber);
  const translationElement = document.getElementById('verseTranslation');
  translationElement.textContent = verse ? verse.TRANSLATION : '(Không tìm thấy bản dịch)';
